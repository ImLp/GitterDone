...
from . import GitUtilities as utilities
...
def generate_wishlist_filemap(wish_list):
    ...
    for path in sorted(wish_list):
        ...
        for i, item in enumerate(split_path):
            ...
            key = temp_path.lower()
            file_map[key] = utilities.Node(temp_path, is_leaf)
...
def get_files_from_directory(directory):
    ...
    for root, dirs, files in os.walk(directory):  # pylint:disable=W0612
        for name in files:
            file_list.append(
                str_utility.normalize_string(os.path.join(root, name))
                )
    return file_list
...
def _get_normalized_files_in_directory(directory: str):
    ...
    files = str_utility.normalize_strings(files, lower=True)
    logging.debug("Files found under '%s' are: %s",
                  directory,
                  str_utility.friendly_list_to_str(files))
    return files
...
def _get_normalized_directories_in_directory(directory: str):
    ...
    directories = str_utility.normalize_strings(directories, lower=True)
    logging.debug("Directories found under '%s' are: %s",
                  directory,
                  str_utility.friendly_list_to_str(directories))
    return directories


def _get_normalized_combined_path(root: str, suffix: str):
    return str_utility.normalize_string(os.path.join(root, suffix))
...
def _cull_files(root_directory: str,
    ...
    if undesired_files:
        logging.debug("Ignoring the following files found under '%s': %s",
                      root_directory,
                      str_utility.friendly_list_to_str(undesired_files))
...
def recursely_get_ignored(root_directory: str,
    ...
    logging.debug(("Checking the entries found under '%s'"
                  ...
                  root_directory,
                  str_utility.friendly_list_to_str(desired_pattern))
    ...
    for folder in raw_directories:
        ...
        if not found_match:
            ...
            path = str_utility.normalize_string(
    ...
    undesired_dirs = sorted(list(set({x.lower() for x in raw_directories} - set(desired_pattern) - set(desired_entries))))
    if undesired_dirs:
        logging.debug("Ignoring the following directories found @ '%s': %s",
                      root_directory,
                      str_utility.friendly_list_to_str(undesired_dirs))
