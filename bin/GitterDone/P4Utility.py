...
from . import GitUtilities as git
from . import ProjectUtilities as project
...
def _revert_unchanged_files_in_p4():
    ...
    """
    command = project.get_p4_tool_path() + ' revert -a'
...
def _checkout_files_in_p4(file_list: list):
    ...
    """
    pretty_checkout_list = str_utility.friendly_list_to_str(file_list)
...
def _checkout_file_in_p4(file_path: str):
    ...
    command = project.get_p4_tool_path() + ' edit \"' + file_path + '\"'
    successfull_command_call, \
        command_results = external_process.trigger_external_subprocess(
            command)
...
def _add_files_to_p4(file_list: list):
    ...
    """
    pretty_add_list = str_utility.friendly_list_to_str(file_list)
...
def _add_file_to_p4(file_path: str):
    ...
    if not file_path:
        ...
        command = project.get_p4_tool_path() + ' add \"' + file_path + '\"'
        successfull_command_call, \
            command_results = external_process.trigger_external_subprocess(
                command)
...
def _delete_files_from_p4(file_list: str):
    ...
    """
    pretty_file_list = str_utility.friendly_list_to_str(file_list)
...
def _delete_file_from_p4(file_path: str):
    ...
    if not file_path:
        ...
        command = project.get_p4_tool_path() + ' delete \"' + file_path + '\"'
        successfull_command_call, \
            command_results = external_process.trigger_external_subprocess(
                command)
...
def _execute_p4_offline_sync(  # pylint:disable=R0912
    ...
    available_git_branches = git.get_git_branch_information()
    ...
    if ignored_branches is not None:
        for ignored_branch in ignored_branches:
            if git.verify_branch_exists_in_git(
                    ignored_branch,
                    available_git_branches):
                ignored_branches_str += f'{ignored_branch.lower()} '

    if not git.verify_branch_exists_in_git(
            desired_branch,
            available_git_branches):
        branches = str_utility.friendly_list_to_str(available_git_branches)
    ...
    files_to_add_or_edit, files_to_delete = git.get_modified_files_for_branch(
        desired_branch,
        include_trunk_branch,
        TRUNK_BRANCH_NAME,
        ignored_branches_str)
...
def _execute_forced_sync_in_p4_to_cl(change_list_number: str,
    ...
    """
    project_root_path = project.get_project_root_path()
    p4_tool_path = project.get_p4_tool_path()

    logging.debug('Forcefully syncing: %s',
                  str_utility.friendly_list_to_str(files_to_sync))

    for entry in files_to_sync:

        path = str_utility.normalize_string(os.path.join(
                        project_root_path,
                        entry),
...
def _execute_git_sync_with_p4(desired_changelist: str,
    ...
    if (not git.check_and_resolve_pending_git_changes(bypass_prompts)
            or not git.check_and_resolve_if_user_is_on_branch(
                TRUNK_BRANCH_NAME,
                bypass_prompts)):
        return

    result, detected_changelist = git.verify_branch_up_to_date(
        TRUNK_BRANCH_NAME,
        desired_changelist,
        forced,
        bypass_prompts)
    ...
    if should_stage_changes:
        ...
        if not git.stage_git_changes():
            return

        commit_message = 'CL ' + desired_changelist
        if not git.commit_git_changes(commit_message):
            return
